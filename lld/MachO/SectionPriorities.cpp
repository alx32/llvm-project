...
macho::PriorityBuilder::getSymbolPriority(const Defined *sym) {
  ...
  const InputFile *f = sym->theCanonicalIsec()->getFile();
  if (!f)
  ...
  for (const InputFile *file : inputFiles) {
    ...
    for (const CallGraphEntry &entry : obj->callGraph) {
      ...
      if (fromSym && toSym &&
        ...
        callGraphProfile[{fromSym->theCanonicalIsec(), toSym->theCanonicalIsec()}] += entry.count;
    }
  }
}
...
macho::PriorityBuilder::buildInputSectionPriorities() {
  ...
  auto addSym = [&](const Defined *sym) {
    ...
    if (!symbolPriority)
      return;
    size_t &priority = sectionPriorities[sym->theCanonicalIsec()];
