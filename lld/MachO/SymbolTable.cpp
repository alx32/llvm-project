...
Defined *SymbolTable::addDefined(StringRef name, InputFile *file,
  ...
  if (!wasInserted) {
    if (auto *defined = dyn_cast<Defined>(s)) {
      if (isWeakDef) {
        ...
        if (auto concatIsec = dyn_cast_or_null<ConcatInputSection>(isec)) {
          ...
          if (defined->theCanonicalIsec())
            transplantSymbolsAtOffset(concatIsec, defined->theCanonicalIsec(),
      ...
      if (defined->isWeakDef()) {
        if (auto concatIsec =
                dyn_cast_or_null<ConcatInputSection>(defined->theCanonicalIsec())) {
          concatIsec->wasCoalesced = true;
          if (isec)
...
Defined *SymbolTable::aliasDefined(Defined *src, StringRef target,
  ...
  return addDefined(target, newFile, src->theCanonicalIsec(), src->value, src->size,
