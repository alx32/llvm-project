...
void UnwindInfoSection::addSymbol(const Defined *d) {
  ...
  auto p = symbols.insert({{d->theCanonicalIsec(), d->value}, d});
  // If we have multiple symbols at the same address, only one of them can have
  // an associated unwind entry.
  if (!p.second && d->theCanUnwindEntry()) {
    assert(p.first->second == d || !p.first->second->theCanUnwindEntry());
    p.first->second = d;
  }
}
...
void UnwindInfoSectionImpl::prepare() {
  ...
  for (const Defined *d : make_second_range(symbols))
    if (d->theCanUnwindEntry()) {
      if (d->theCanUnwindEntry()->getName() == section_names::compactUnwind) {
        prepareRelocations(d->theCanUnwindEntry());
      } else {
        ...
        FDE &fde = cast<ObjFile>(d->getFile())->fdes[d->theCanUnwindEntry()];
  ...
  for (size_t i = 0; i < isec->relocs.size(); ++i) {
    ...
    if (auto *s = r.referent.dyn_cast<Symbol *>()) {
      ...
      if (auto *defined = dyn_cast<Defined>(s)) {
        ...
        Symbol *&personality =
            personalityTable[{defined->theCanonicalIsec(), defined->value}];
...
Symbol *UnwindInfoSectionImpl::canonicalizePersonality(Symbol *personality) {
  if (auto *defined = dyn_cast_or_null<Defined>(personality)) {
    // Check if we have created a synthetic symbol at the same address.
    Symbol *&synth = personalityTable[{defined->theCanonicalIsec(), defined->value}];
    if (synth == nullptr)
      synth = defined;
...
void UnwindInfoSectionImpl::relocateCompactUnwind(
  ...
  parallelFor(0, symbolsVec.size(), [&](size_t i) {
    ...
    // to it.
    if (d->theCanUnwindEntry()->getName() == section_names::ehFrame) {
      ...
      uint64_t dwarfOffsetHint =
          d->theCanUnwindEntry()->outSecOff <= DWARF_SECTION_OFFSET
              ? d->theCanUnwindEntry()->outSecOff
              : 0;
      cu.encoding = target->modeDwarfEncoding | dwarfOffsetHint;
      const FDE &fde = cast<ObjFile>(d->getFile())->fdes[d->theCanUnwindEntry()];
    ...
    assert(d->theCanUnwindEntry()->getName() == section_names::compactUnwind);

    auto buf = reinterpret_cast<const uint8_t *>(d->theCanUnwindEntry()->data.data()) -
               target->wordSize;
    cu.functionLength =
        support::endian::read32le(buf + cuLayout.functionLengthOffset);
    cu.encoding = support::endian::read32le(buf + cuLayout.encodingOffset);
    for (const Reloc &r : d->theCanUnwindEntry()->relocs) {
      if (r.offset == cuLayout.personalityOffset)
