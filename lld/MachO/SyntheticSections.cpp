  ...
  for (ConcatInputSection *isec : inputSections) {
    ...
    if (const auto *sym = Reloc.referent.dyn_cast<Symbol *>()) {
      if (const auto *d = dyn_cast<Defined>(sym)) {
        auto *cisec = cast<CStringInputSection>(d->theCanonicalIsec());
    ...
    if (auto *objFile = dyn_cast<ObjFile>(file)) {
      for (const Symbol *sym : objFile->symbols) {
        if (const auto *defined = dyn_cast_or_null<Defined>(sym)) {
          if (!defined->theCanonicalIsec() || !isCodeSection(defined->theCanonicalIsec()) ||
              !defined->isLive())
            continue;
...
void SymtabSection::emitStabs() {
  ...
  for (const SymtabEntry &entry :
    ...
    if (auto *defined = dyn_cast<Defined>(sym)) {
      ...
      symbolsNeedingStabs.emplace_back(defined, defined->theCanonicalIsec()->getFile()->id);
    }
  }
  ...
  for (SortingPair &pair : symbolsNeedingStabs) {
    ...
    InputSection *isec = defined->theCanonicalIsec();
    ...
    StabsEntry symStab;
    symStab.sect = defined->theCanonicalIsec()->parent->index;
  ...
  for (const SymtabEntry &entry : concat<const SymtabEntry>(
    ...
    if (auto *defined = dyn_cast<Defined>(entry.sym)) {
      ...
      if (defined->isAbsolute()) {
        ...
        nList->n_sect = defined->theCanonicalIsec()->parent->index;
...
void ObjCMethListSection::setUp() {
  for (const ConcatInputSection *isec : inputs) {
    ...
    while (methodNameOff < isec->data.size()) {
      ...
      auto *cisec = cast<CStringInputSection>(def->theCanUnwindEntry());
...
void ObjCMethListSection::writeRelativeOffsetForIsec(
  ...
  if (useSelRef) {
    auto *cisec = cast<CStringInputSection>(def->theCanonicalIsec());
    auto methname = cisec->getStringRefAtOffset(def->value);
