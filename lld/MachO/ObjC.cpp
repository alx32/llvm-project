...
static StringRef getReferentString(const Reloc &r) {
  ...
  return cast<CStringInputSection>(sym->theCanonicalIsec())->getStringRefAtOffset(sym->value +
                                                                    r.addend);
}
...
void ObjcCategoryChecker::parseClass(const Defined *classSym) {
  ...
  const auto *classIsec = cast<ConcatInputSection>(classSym->theCanonicalIsec());
  ...
  if (const auto *r = classSym->theCanonicalIsec()->getRelocAt(classLayout.metaClassOffset))
...
void ObjcCategoryMerger::tryEraseDefinedAtIsecOffset(
  ...
  if (auto *cisec = dyn_cast_or_null<ConcatInputSection>(sym->theCanonicalIsec()))
    eraseISec(cisec);
  else if (auto *csisec = dyn_cast_or_null<CStringInputSection>(sym->theCanonicalIsec())) {
...
void ObjcCategoryMerger::collectCategoryWriterInfoFromCategory(
  ...
  if (!infoCategoryWriter.catNameInfo.valid) {
    ...
    collectSectionWriteInfoFromIsec<CStringSection>(
        catNameSym->theCanonicalIsec(), infoCategoryWriter.catNameInfo);
  }
  ...
  if (!infoCategoryWriter.catPtrListInfo.valid) {
    for (uint32_t off = catLayout.instanceMethodsOffset;
      ...
      if (Defined *ptrList =
        ...
        collectSectionWriteInfoFromIsec<ConcatOutputSection>(
            ptrList->theCanonicalIsec(), infoCategoryWriter.catPtrListInfo);
...
void ObjcCategoryMerger::parseProtocolListInfo(const ConcatInputSection *isec,
  ...
  uint32_t protocolCount = *reinterpret_cast<const uint32_t *>(
      ptrListSym->theCanonicalIsec()->data.data() + listHeaderLayout.structSizeOffset);
  ...
  assert(expectedListSize == ptrListSym->theCanonicalIsec()->data.size() &&
  ...
  for (uint32_t inx = 0; inx < protocolCount; ++inx) {
    const Reloc *reloc = ptrListSym->theCanonicalIsec()->getRelocAt(off);
  ...
  assert((ptrListSym->theCanonicalIsec()->getRelocAt(off) == nullptr) &&
...
void ObjcCategoryMerger::parsePointerListInfo(const ConcatInputSection *isec,
  ...
  uint32_t thisStructCount = *reinterpret_cast<const uint32_t *>(
      ptrListSym->theCanonicalIsec()->data.data() + listHeaderLayout.structCountOffset);
...
void ObjcCategoryMerger::createSymbolReference(Defined *refFrom,
  ...
  refFrom->theCanonicalIsec()->relocs.push_back(r);
}
...
void ObjcCategoryMerger::collectAndValidateCategoriesData() {
  for (InputSection *sec : allInputSections) {
    ...
    for (uint32_t off = 0; off < catListCisec->getSize();
      ...
      auto *catBodyIsec = dyn_cast<ConcatInputSection>(categorySym->theCanonicalIsec());
      assert(catBodyIsec &&
